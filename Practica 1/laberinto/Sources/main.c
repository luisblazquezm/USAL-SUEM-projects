...
// led color macros
#define LED_COLOR_WINNER 4
#define LED_COLOR_GAME_ON 5
#define LED_COLOR_GAME_OFF 6
...
#define COLOR_WINNER clBlue
#define COLOR_WALL clRed


/*lint -save  -e970 Disable MISRA rule (6.3) checking. */

volatile int mytimer = TIMER_VALUE;
volatile bool gameFinished = TRUE;
volatile bool gameFinishedBecauseOfTimer = FALSE;
int p1[2], p2[2];
...
void plotTimeAndPoints(){
	...
	sprintf(buffer, "TIEMPO: %03d", mytimer);
	...
	// plot player 1
	memset(buffer,0,sizeof(char)*80);
	sprintf(buffer, "P1: %02d", numPointsP1);
	Term1_MoveTo(22,1);
	Term1_SendStr(buffer);

	// plot player 2
	memset(buffer,0,sizeof(char)*80);
	sprintf(buffer, "P2: %02d", numPointsP2);
	Term1_MoveTo(30,1);
	Term1_SendStr(buffer);
}
...
/*lint -restore Enable MISRA rule (6.3) checking. */
{
  ...
  while(1){
	  ...
	  Bits_RGB_PutVal(LED_COLOR_GAME_OFF);
	  waitForGameToStart();

	  // set game flat to started and change led color
	  gameFinished = FALSE;
	  Bits_RGB_PutVal(LED_COLOR_GAME_ON);
	  ...
	  while(!gameFinished){
		  ...
		  if(gameFinished){
			  ...
			  // update led color
			  Bits_RGB_PutVal(LED_COLOR_WINNER);

			  // update time and points and play tone
			  plotTimeAndPoints();
			  playtone(TONES_WINNER_T, TONES_WINNER_L, 1000);
